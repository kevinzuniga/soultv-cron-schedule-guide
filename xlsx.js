...
for (let i = 1; i < data.length; i++) {
  const row = data[i];
  if (!row[1] || !row[2] || !row[3] || (row[3] === 'PROGRAMA') ) continue; // Saltar filas incompletas

  const [dayOfWeek, date, startTime, program, genre, classification] = row.slice(1, 7);
  ...
  const formattedStopTime = nextRow && nextRow[2] ? excelTimeToString(nextRow[3]) : '23:59:59';
